...
class User
{
    ...
    // Metoda, ki posodobi geslo trenutnega uporabnika v bazi
    public function update_password($password){
        $db = Db::getInstance();
        $pass = password_hash($password, PASSWORD_DEFAULT);
        $id = $this->id;
        $query = "UPDATE users SET password='$pass' WHERE id=$id LIMIT 1;";
        if($db->query($query)){
            return true;
        }
        else{
    ...
    // Metoda ki prešteje število objav uporabnika
    public static function count_articles($id){
        $db = Db::getInstance();
        $id = mysqli_real_escape_string($db, $id);
        $query = "SELECT COUNT(*) as count FROM articles WHERE user_id = '$id';";
        $res = $db->query($query);
        $count = $res->fetch_object();
        return $count->count;
    }

    // Metoda ki vrne število komentarjev uporabnika
    public static function count_comments($id){
        $db = Db::getInstance();
        $id = mysqli_real_escape_string($db, $id);
        $query = "SELECT COUNT(*) as count FROM comments WHERE user_id = '$id';";
        $res = $db->query($query);
        $count = $res->fetch_object();
        return $count->count;
    }
