...
require_once 'comments.php'; // Vključimo model za komentarje
...
class Article
{
    ...
    public static function create($title, $abstract, $text, $user){
        $db = Db::getInstance();
        $title = mysqli_real_escape_string($db, $title);
        $abstract = mysqli_real_escape_string($db, $abstract);
        $text = mysqli_real_escape_string($db, $text);
        $user = mysqli_real_escape_string($db, $user);
        $query = "INSERT INTO articles (title, abstract, text, user_id) VALUES ('$title', '$abstract', '$text', '$user');";
        if($db->query($query)){
            return true;
        }
        else{
    ...
    public static function myArticles($user)
    {
        $db = Db::getInstance(); // pridobimo instanco baze
        $query = "SELECT * FROM articles WHERE user_id = '$user'"; // pripravimo query
        $res = $db->query($query); // poženemo query
        $articles = array();
        while ($article = $res->fetch_object()) {
            array_push($articles, new Article($article->id, $article->title, $article->abstract, $article->text, $article->date, $article->user_id));
