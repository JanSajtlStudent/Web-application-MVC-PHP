...
$controllers = array(
  'pages' => ['error', 'profile_error'],
  'users' => ['create', 'store' , 'password', 'profile'],
  'auth' => ['login', 'authenticate'],
  'articles' => ['index', 'show', 'create', 'list', 'update', 'delete'],
  'comments' => ['create', 'store']
);
// Če je prijavljen, mu dovolimo še urejanje profila, odjavo in objavo novic
if(isset($_SESSION["USER_ID"])){
  $controllers['users'] = array_merge($controllers['users'], ['edit', 'update'], ['password'], ['profile']);
  $controllers['auth'] = array_merge($controllers['auth'], ['logout']);
  $controllers['articles'] = array_merge($controllers['articles'], ['create'], ['update'], ['delete']); // TODO: 'list', 'store', 'edit', 'update', 'delete'
  $controllers['comments'] = array_merge($controllers['comments'], ['all']);
}
